...
def process_for_video():
	fwx = open("data/video_taurus_kinematics_feature_X.txt", "w")
	fwy = open("data/video_taurus_kinematics_feature_y.txt", "w")
	count = 0
	# sample_rate = 5

	prev_label = "0"
	with open("data/taurus_kinematics.txt") as f:
		for line in f:
			line = line.strip()
			line = line.replace("\n", "")
			line = line.replace("\r", "")
...
def merge_kinematic_video():
	fwx = open("/scratch/gilbreth/rahman64/dataset/vae/Taurus_vae/taurus_kinematics_video_feature_all_1_X.txt", "w")
	fwy = open("/scratch/gilbreth/rahman64/dataset/vae/Taurus_vae/taurus_kinematics_video_feature_all_1_y.txt", "w")
	count = 0
	# sample_rate = 20
	prev_label = "0"
	kindata = {}
	with open("data/video_taurus_kinematics_feature_X.txt") as f:
		for line in f:
			line = line.strip()
			line = line.replace("\n", "")
			line = line.replace("\r", "")
	...
	frameids = []
	with open("/scratch/gilbreth/rahman64/dataset/vae/Taurus_vae/frameid_sorted_nodiff_all_1.txt") as f:
		for line in f:
			line = line.strip()
			line = line.replace("\n", "")
			line = line.replace("\r", "")
	...
	features = np.loadtxt("/scratch/gilbreth/rahman64/dataset/vae/Taurus_vae/features_sorted_nodiff_all_1.cnnvae", delimiter=' ')
	XT, yT = features[:, :-1], features[:, -1]
	print("no Applying pca")
...
def merge_unify_kin_video(video=True, kin=True):
	A = np.loadtxt("/scratch/gilbreth/rahman64/dataset/vae/Taurus_vae/taurus_kinematics_video_feature_all_1_X.txt", delimiter = ' ')
	A1 = A[:, :19] # add int for python 3?
	A2 = A[:, 19:38]
	A3 = A[:, 38:]
	...
	if video == True and kin == True:
		...
		print(A_new.shape)
		np.savetxt('/scratch/gilbreth/rahman64/dataset/vae/Taurus_vae/taurus_kinematics_video_feature_all_1_X_unify.txt', A_new, delimiter=' ')
	elif video == False and kin == True:
		A_new = np.concatenate((A1, A2), axis = 1)
		print(A_new.shape)
		np.savetxt('/scratch/gilbreth/rahman64/dataset/vae/Taurus_vae/taurus_kinematics_only_feature_all_1_X_unify.txt', A_new, delimiter=' ')
	elif video == True and kin == False:
		A_new = A3
		print(A_new.shape)
		np.savetxt('/scratch/gilbreth/rahman64/dataset/vae/Taurus_vae/taurus_video_only_feature_all_1_X_unify.txt', A_new, delimiter=' ')
